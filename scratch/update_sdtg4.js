...
const targetAssemblyName = "SD-TG-4(WITH GROUND PLATE IN POLE)TANGENT STRUCTURE,ADJACENT TO DEADEND POLE";
...
const newItems = [
  ...
  { "code": "1010180100", "name": "WASHER,PLAIN,SQUARE,LARGE 52x52x4.5 mm.HOLE DIA. 18 MM. TIS.258", "unit": "ชิ้น", "qty": 30 },
...
const targetAssemblyIndex = data.findIndex(a => a.assemblyName === targetAssemblyName);

if (targetAssemblyIndex !== -1) {
  data[targetAssemblyIndex].items = newItems;
  fs.writeFileSync(dataPath, JSON.stringify(data, null, 2), 'utf-8');
  console.log(`Successfully updated items for assembly: ${targetAssemblyName}`);
} else {
  console.log(`Could not find assembly: ${targetAssemblyName}`);
}
